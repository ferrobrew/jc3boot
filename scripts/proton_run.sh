...
# tools (a payload's injector) can inject into the running game later.
#
...
#
# The service claims JC3_BUS_NAME on the session bus. A consumer's launch client
# must connect to that exact name, so it either uses the same default or passes
# the same JC3_BUS_NAME to both this script and its own launch step.
#
...
BUS="${JC3_BUS_NAME:-com.jc3boot.JustCause3}"
